...
#region Unity Messages
	...
	protected virtual void UpdateAnchors(bool updateEyeAnchors, bool updateHandAnchors)
	{
		...
		if (updateEyeAnchors)
		{
			...
			// Set up center eye based on headset or no headset
			if (hmdPresent)
			{
			...
			// Set up left and right eye position and rotations
			// Set up for not having a headset, or for monoscopic display
			if (!hmdPresent || monoscopic)
			{
				// If we are not using two HMD eyes, every eye follows the center eye.
			...
			{
				// We have a headset and are using stereoscopic display
				// Use headset information to set eyes
		...
		{
			//Need this for controller offset because if we're on OpenVR, we want to set the local poses as specified by Unity,
			//but if we're not, OVRInput local position is the right anchor
			if (OVRManager.loadedXRDevice == OVRManager.XRDevice.OpenVR)
			{
				// OpenVR things
				// If it's OpenVR (not just oculus headset), you have to do some math functions with properties that all OpenVR headsets have
			...
			{
				// Get controller position and rotation from OVRInput package
				// Oculus has easy position and rotation getters
				// This is oculus only
			...
			// OVRPose only needs to be taken into account if using OpenVR and not an oculus
	...
	{
		// Just control if we need to update the hand anchors
